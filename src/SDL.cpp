...
namespace SDL
{
  ...
  Coord screenToCartesian(const Point& screenCoord,
			  ...
			  const Coord& offset)
  {
    Coord cartesianCoord {screenCoord.getX() - width/2.0, height/2.0 - screenCoord.getY()};
			  ...
			  double scale)
  {
    ...
    return {static_cast<int>(std::round(std::real(screenCoord))), static_cast<int>(std::round(std::imag(screenCoord)))};
  }
